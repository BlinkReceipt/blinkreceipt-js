...
BlinkReceipt.onCreateUI = function($parentContainer, $elemCenter) {
    ...
    var scanFrame = $('<div id="scanFrame" style="display:none;"></div>')
    $parentContainer.append(scanFrame);

    var $elemDivButtonBar = $('<div id="brjs-divButtonBar">');
    var $elemTableButtons = $('<table border=0 width=100% id="brjs-tblButtons">');
    var $elemRowButtons = $('<tbody><tr><td align="center"><button id="brjs-btnSecondaryAction" class="brjs-actionButton">Cancel</button></td><td align="center"><button id="brjs-snap" class="brjs-cameraButton"></button></td><td align="center"><button id="brjs-finish" class="brjs-actionButton">Finish</button></td></tr></tbody>');
...
BlinkReceipt.onRetakeScan = function() {
    ...
    if (BlinkReceipt.staticImages.length > 0) {
        var lastImg = BlinkReceipt.staticImages.pop();
        lastImg.remove();
    }
...
BlinkReceipt.onAddScanButtonClicked = function() {
    ...
    if (BlinkReceipt.staticImages.length > 1) {
        var prevImg = BlinkReceipt.staticImages[BlinkReceipt.staticImages.length-2];
        prevImg.detach().prependTo($('#brjs-container'));
    }

    if (BlinkReceipt.staticImages.length > 0) {
        var curImg = BlinkReceipt.staticImages[BlinkReceipt.staticImages.length-1];
        curImg.animate({
