...
var BlinkReceipt = {
     ...
     * @param parseResults {object} For the structure of the results object consult the response schema in the API docs at {@link https://app.swaggerhub.com/apis-docs/blinkreceipt/apiscan/0.1}
     */
    onFinished: function(parseResults) {
    ...
    createUI: function() {
        ...
        //hack to get location of js file
        var jsFileLocation = $('script[src*=blinkreceipt]').attr('src');  // the js file path
        jsFileLocation = jsFileLocation.replace(/blinkreceipt\.js.*$/, ''); 

        this.audio = new Audio(jsFileLocation + '../media/camera.wav');
    },
    ...
    returnResults: function() {
        this.onFinished(this.parseResults);
    ...
    postImagesToServer: function() {
        this.frames.forEach(function(curFrame, idx) {
            ...
            data.append('receipt_images[image]', curFrame);
